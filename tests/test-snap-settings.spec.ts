...
test.describe('스냅 설정 UI 및 사용자 제어 기능 테스트', () => {
  ...
  test('편집 모드 진입 및 스냅 설정 UI 확인', async ({ page }) => {
    ...
    // 그리드 스냅 토글 버튼 확인 (더 구체적인 선택자 사용)
    await expect(page.locator('button:has-text("🔲"):has-text("그리드")')).toBeVisible();
    
    // 회전 스냅 토글 버튼 확인 (더 구체적인 선택자 사용)
    await expect(page.locator('button:has-text("⚡"):has-text("회전")')).toBeVisible();
  ...
  test('스냅 강도 조절 기능 테스트', async ({ page }) => {
    ...
    // 그리드 알림이 여전히 표시되는지 확인하고, 있다면 더 기다림
    const gridNotification = page.locator('text=그리드 활성화됨');
    if (await gridNotification.isVisible()) {
      await page.waitForTimeout(5000);
    }
    
    // 스냅 강도 조절 버튼이 클릭 가능한 상태인지 확인
    const snapButton = page.locator('button:has-text("🎯")');
  ...
  test('그리드 스냅 토글 기능 테스트', async ({ page }) => {
    ...
    // 그리드 스냅 버튼 클릭 (더 구체적인 선택자 사용)
    const gridButton = page.locator('button:has-text("🔲"):has-text("그리드")');
  ...
  test('회전 스냅 토글 기능 테스트', async ({ page }) => {
    ...
    // 회전 스냅 버튼 클릭 (더 구체적인 선택자 사용)
    const rotationButton = page.locator('button:has-text("⚡"):has-text("회전")');
