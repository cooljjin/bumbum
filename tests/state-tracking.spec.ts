...
test('편집 모드 상태 추적 테스트', async ({ page }) => {
  // 페이지 로드
  ...
  // 페이지의 HTML에서 "편집 모드" 텍스트가 포함된 요소 찾기
  const editModeElements = page.locator('*:has-text("편집 모드")');
  console.log('"편집 모드" 텍스트를 포함한 요소 개수:', await editModeElements.count());
  
  // 페이지의 HTML에서 "뷰 모드" 텍스트가 포함된 요소 찾기
  const viewModeElements = page.locator('*:has-text("뷰 모드")');
  console.log('"뷰 모드" 텍스트를 포함한 요소 개수:', await viewModeElements.count());
  
  // 페이지 스크린샷 저장
  await page.screenshot({ path: 'test-results/state-tracking.png' });
