...
test('현재 페이지 구조 디버깅', async ({ page }) => {
  ...
  // 3D 관련 텍스트가 있는지 확인
  const has3DText = pageContent.includes('3D') || pageContent.includes('Three.js') || pageContent.includes('WebGL');
  console.log('3D 관련 텍스트 존재:', has3DText);
});
